...
@ensure_annotations
def read_csv(path: Path, delimiter: str = ',', header: bool = True) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        header (bool, optional): Indicates if the first row of the CSV is a header. Defaults to True.

    Raises:
        FileNotFoundError: If the CSV file is not found.
        pd.errors.ParserError: If there's an error in parsing the CSV file.
    ...
    try:
        ...
        logger.error(f"Error parsing CSV file at {path}. Error: {e}")
        raise
    except PermissionError:
        logger.error(f"Permission denied to read {path}")
        raise
    except Exception as e:
        logger.error(f"An unexpected error occurred while reading {path}. Error: {e}")
        raise
